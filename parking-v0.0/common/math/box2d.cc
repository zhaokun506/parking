  ...
  namespace common
  {
    namespace math
    {
      ...
      bool Box2d::HasOverlap(const LineSegment2d &line_segment) const
      {
        ...
        {
          return IsPointIn(line_segment.start()); //如果线段长度过小看做点
        }
        //如果在安全包围盒外则不碰撞
      ...
      double Box2d::DistanceTo(const LineSegment2d &line_segment) const
      {
        if (line_segment.length() <= kMathEpsilon)
        {
          return DistanceTo(line_segment.start()); //如果线段长度很小，看做点
        }
        //判断起点是否在矩形内
        const double ref_x1 = line_segment.start().x() - center_.x();
        const double ref_y1 = line_segment.start().y() - center_.y();
        double x1 = ref_x1 * cos_heading_ + ref_y1 * sin_heading_; //与方向向量点乘，点到宽度方向的距离
        double y1 = ref_x1 * sin_heading_ - ref_y1 * cos_heading_; //与方向向量叉乘，点到长度方向的距离
        double box_x = half_length_;                               //车长
        double box_y = half_width_;                                //车宽
        int gx1 = (x1 >= box_x ? 1 : (x1 <= -box_x ? -1 : 0));     //投影点在车长Xv范围内(1,-1)
        int gy1 = (y1 >= box_y ? 1 : (y1 <= -box_y ? -1 : 0));     //投影点在车宽Yv范围内
        if (gx1 == 0 && gy1 == 0)
        {
          return 0.0;
        }
