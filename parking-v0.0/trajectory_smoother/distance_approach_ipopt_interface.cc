...
bool DistanceApproachIPOPTInterface::get_nlp_info(int& n, int& m,
  ...
  ADEBUG << "get_nlp_info";
  // n1 : states variables, 4 * (N+1) 状态变量
  int n1 = 4 * (horizon_ + 1);
  ADEBUG << "n1: " << n1;
  // n2 : control inputs variables 控制变量
  int n2 = 2 * horizon_;
  ADEBUG << "n2: " << n2;
  // n3 : sampling time variables 时间变量
  int n3 = horizon_ + 1;
  ADEBUG << "n3: " << n3;
  // n4 : dual multiplier associated with obstacle shape λ =障碍物边的数量*n
  lambda_horizon_ = obstacles_edges_num_.sum() * (horizon_ + 1);
  ADEBUG << "lambda_horizon_: " << lambda_horizon_;
  // n5 : dual multipier associated with car shape, obstacles_num*4 * (N+1) μ
  // =障碍物的数量*4
  ...
  int m1 = 4 * horizon_;
  // m2 : control rate constraints (only steering) 控制变化率约束
  int m2 = horizon_;
  // m3 : sampling time equality constraints 采样时间约束
  int m3 = horizon_;
  // m4 : obstacle constraints 障碍物约束
...
void DistanceApproachIPOPTInterface::eval_constraints(int n, const T* x, int m,
  ...
  // 3. Time constraints 1 * [0, horizons-1]//时间约束
  time_index = time_start_index_;
  for (int i = 0; i < horizon_; ++i) {
