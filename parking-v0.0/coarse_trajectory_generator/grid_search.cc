...
bool GridSearch::GenerateAStarPath(
  ...
  obstacles_linesegments_vec_ = obstacles_linesegments_vec;//参数转为私有变量是为了让类中可以使用该变量
  open_set.emplace(start_node->GetIndex(), start_node);//unorderer_map(x_y索引,node2d node)
  open_pq.emplace(start_node->GetIndex(), start_node->GetCost());//(string,double)
...
bool GridSearch::GenerateDpMap(
  ...
  while (!open_pq.empty()) {
    ...
    open_pq.pop();
    std::shared_ptr<Node2d> current_node = open_set[current_id];
    dp_map_.emplace(current_node->GetIndex(), current_node);//当前节点节点存入dp_map_
    std::vector<std::shared_ptr<Node2d>> next_nodes =
        std::move(GenerateNextNodes(current_node));//生成扩展节点,并计算其cost（从起点到该点的距离cost）
    for (auto& next_node : next_nodes) {
      if (!CheckConstraints(next_node)) {//检查是否超过边界或者碰撞，则跳过
        continue;
      }
      if (dp_map_.find(next_node->GetIndex()) != dp_map_.end()) {//map如果搜索不到会返回末尾迭代器
        continue;//如果节点存在于close_set()内，则跳过
      }
      if (open_set.find(next_node->GetIndex()) == open_set.end()) {//如果不在open_set
        ++explored_node_num;
