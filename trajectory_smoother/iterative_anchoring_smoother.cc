...
bool IterativeAnchoringSmoother::Smooth(
  ...
  if (!SetPathProfile(interpolated_warm_start_point2ds,
    ...
    std::cout << "Set path profile fails";
    return false;
  }
  ...
  if (!CheckCollisionAvoidance(interpolated_warm_start_path,
    ...
    //   std::cout << "Fail to reanchor colliding input path points";
    //   return false;
    // }
  }
  ...
  std::cout << "discretized_trajectory size " << discretized_trajectory->size();
  return true;
}
...
bool IterativeAnchoringSmoother::ReAnchoring(
  ...
  if (colliding_point_index.empty()) {
    std::cout << "no point needs to be re-anchored";
    return true;
  }
  ...
  for (const auto index : colliding_point_index) {
    if (index == 0 || index == path_points->size() - 1) {
      std::cout << "Initial and end points collision avoid condition failed.";
      return false;
    }
...
bool IterativeAnchoringSmoother::SmoothPath(
  ...
  while (!is_collision_free) {
    ...
    std::cout << "loop iteration number is " << counter;
    ++counter;
  }
  return true;
}
...
bool IterativeAnchoringSmoother::SmoothSpeed(const double init_a,
  ...
  const double delta_t =
      planner_open_space_config_.iterative_anchoring_smoother_config.delta_t;

  const double total_t = 2 * path_length / max_reverse_acc * 10;
  std::cout << "total_t is : " << total_t;
  ...
  auto s_curve_config = planner_open_space_config_
                            .iterative_anchoring_smoother_config.s_curve_config;
...
bool IterativeAnchoringSmoother::CombinePathAndSpeed(
  ...
  std::cout << "speed_points.TotalTime() " << speed_points.TotalTime();
  ...
  std::cout << "path length before combine " << path_points.Length();
...
bool IterativeAnchoringSmoother::GenerateStopProfileFromPolynomial(
  ...
  std::cout << "GenerateStopProfileFromPolynomial fails.";
  return false;
}
...
double
IterativeAnchoringSmoother::CalcHeadings(const DiscretizedPath &path_points,
                                         const size_t index) {
