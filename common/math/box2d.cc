...
double Box2d::DistanceTo(const LineSegment2d &line_segment) const {
  ...
  double x1 = ref_x1 * cos_heading_ +
              ref_y1 * sin_heading_; //与方向向量点乘，点到宽度方向的距离
  double y1 = ref_x1 * sin_heading_ -
              ref_y1 * cos_heading_; //与方向向量叉乘，点到长度方向的距离
  double box_x = half_length_; //车长
  double box_y = half_width_;  //车宽
  int gx1 =
      (x1 >= box_x ? 1 : (x1 <= -box_x ? -1 : 0)); //投影点在车长Xv范围内(1,-1)
