...
/*
C++ 标准库使用比如vector::push_back
等这类函数时,会对参数的对象进行复制,连数据也会复制.这就会造成对象内存的额外创建,
本来原意是想把参数push_back进去就行了. C++11 提供了std::move
函数来把左值转换为右值,
而且新版的push_back也支持&&参数的重载版本,这时候就可以高效率的使用内存了.
对指针类型的标准库对象并不需要这么做.
//构造函数的初始化列表，调用拷贝构造。: A值（A参数名），B值（B参数名）
*/
...
void Polygon2d::BuildFromPoints() {
  num_points_ = static_cast<int>(points_.size()); //获取顶点向量的数量
  ...
  for (int i = 1; i < num_points_; ++i) {
    ...
    area_ += CrossProd(points_[0], points_[i - 1],
                       points_[i]); //同一起点的两个向量叉乘
  }
  if (area_ < 0) {
    area_ = -area_;
    // begin()第一个位置的迭代器，end()最后一个位置的下一个迭代器
    std::reverse(points_.begin(), points_.end()); //翻转.begin->end的所有元素
  }
  area_ /= 2.0; //等价于a=a/b;
