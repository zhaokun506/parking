...
class ApolloOpenSpaceTrajectoryOptimizer {
public:
  ApolloOpenSpaceTrajectoryOptimizer(
      const OpenSpaceTrajectoryOptimizerConfig &config);

  virtual ~ApolloOpenSpaceTrajectoryOptimizer() = default;

  bool Plan(const std::vector<common::TrajectoryPoint> &stitching_trajectory,
  ...
  void GetCoarseTrajectory(DiscretizedTrajectory *coarse_trajectory) {
...
private:
  bool
  IsInitPointNearDestination(const common::TrajectoryPoint &planning_init_point,
                             const std::vector<double> &end_pose,
                             double rotate_angle,
                             const common::math::Vec2d &translate_origin);
...
private:
  ...
  std::unique_ptr<DistanceApproachApolloProblem> distance_approach_;
